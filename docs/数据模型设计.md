...
- **数据库**：SQLite 3（WAL 模式）
- **加密算法**：AES-256-CBC
- **密钥派生**：PBKDF2（salt='salt'，迭代=10000，长度=32，`sha256`）
- **访问层**：原生 SQL + better-sqlite3
...
CREATE TABLE IF NOT EXISTS passwords (
    ...
    password TEXT NOT NULL,     -- 单账号模型，必填并加密存储
    url TEXT,
    notes TEXT,
    group_id INTEGER,           -- 可为空，删除分组时置 NULL
    created_at TEXT NOT NULL,
    updated_at TEXT NOT NULL,
...
（变更）移除“多账号密码文本格式”，自由文本说明迁移至“便笺”模块（SecureRecord）。
...
- 密码条目更新时，支持分组变更
- 支持全文搜索（title、username、url、notes）
- 自动维护创建时间和更新时间
...
### 2.4 便笺模型 (SecureRecord)

#### 数据表结构
```sql
CREATE TABLE secure_record_groups (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    name TEXT NOT NULL,
    parent_id INTEGER,                 -- 支持树形分组（自关联）
    color TEXT DEFAULT 'blue',         -- 分组颜色
...
CREATE TABLE secure_records (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title TEXT,                    -- 标题可明文，便于列表与索引
    content_ciphertext TEXT NOT NULL, -- 正文密文，端侧加密
    group_id INTEGER,              -- 独立分组体系
...
interface SecureRecord {
  id?: number;
  title?: string;               // 标题（可选明文）
  content_ciphertext: string;   // 正文密文
  group_id?: number;            // 分组（可为空）
  pinned?: boolean;
  archived?: boolean;
  created_at?: string;
  updated_at?: string;
...
#### 业务规则
- 自由文本内容全部端侧加密，服务端不可读，不参与服务端全文索引。
- 搜索在客户端解密后进行；为性能考虑采用分页解密与本地缓存。
- 分组独立于密码分组；删除分组时将记录的 group_id 置 NULL。
- 支持置顶/归档标记与列表筛选。
 - 便笺分组支持树形结构（parent_id 自关联），名称在同一父级下唯一。
 - 分组支持颜色设置（预置色与自定义值），用于列表与标签展示。
 - 逻辑关联：不使用数据库外键，所有约束与级联由应用层维护（如删除分组时将子分组与记录的引用置空或迁移）。
...
```
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐     ┌──────────────────────────┐
│  user_settings  │     │     groups      │     │   passwords     │     │  secure_record_groups     │
├─────────────────┤     ├─────────────────┤     ├─────────────────┤     ├──────────────────────────┤
│ id (PK)         │     │ id (PK)         │─────│ id (PK)         │     │ id (PK)                  │
│ key (UNIQUE)    │     │ name            │     │ title           │     │ name                     │
│ value           │     │ parent_id       │     │ username        │     │ sort_order               │
│ type            │     │ color           │     │ password        │     │ created_at               │
│ category        │     │ created_at      │     │ url             │     │ updated_at               │
...
// PBKDF2 派生 32 字节密钥（salt='salt', 10000 次，sha256）
...
function encrypt(text: string): string {
  ...
  return iv.toString('hex') + ':' + encrypted; // ivHex:cipherHex
}
...
function decrypt(encryptedText: string): string {
  try {
    ...
    if (parts.length !== 2) return encryptedText; // 非加密文本兼容
...
### 4.3 加密范围
必须加密：`passwords.password`、`password_history.old_password/new_password`、`secure_records.content_ciphertext`
不加密：ID、时间戳、分组名称、颜色、索引字段
...
CREATE TRIGGER IF NOT EXISTS passwords_fts_update AFTER UPDATE ON passwords BEGIN
  UPDATE passwords_fts SET title=new.title, username=new.username, url=new.url, notes=new.notes
  WHERE rowid=new.id;
END;
...
#### 加密压缩实现
当前仓库提供 `encrypted_zip` 的简化实现：将 JSON 字符串加密为 `ivHex:cipherHex` 并附带元数据文本（非真实 ZIP）。后续计划完善为 ZIP+AES 的标准实现。
